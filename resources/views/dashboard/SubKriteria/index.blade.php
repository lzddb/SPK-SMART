            ...
            <table class="table table-bordered" width="100%" cellspacing="0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td>{{ $c->bobot_sub }}</td>
                        <td>
                            <a href="" class="btn btn-warning btn-sm mr-1" data-bs-toggle="modal" data-bs-target="#updateSubKriteria-{{ $c->id }}"> <i class="fas fa-pen"></i>
                            </a>
                            <a href="{{ route('delete-sub-kriteria', $c->id) }}" class="btn btn-danger btn-sm mr-1">
...
<div class="modal fade" id="addSubKriteria" tabindex="-1" aria-labelledby="addSubKriteriaModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <form action="{{ route('tambah-sub-kriteria') }}" method="POST">
                @csrf
...
<div class="modal fade" id="updateSubKriteria-{{ $item->id }}" tabindex="-1" aria-labelledby="UpdateSubKriteriaModal" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            ...
            <form action="{{ route('update-sub-kriteria', $item->id) }}" method="POST">
                @method('PUT')
                @csrf
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="id_kriteria" class="form-label">Kriteria</label>
                        <input type="text" value="{{ $item->id_kriteria }}" class="form-control" name="id_kriteria" readonly>
                    </div>
                    <div class="mb-3">
                        <label for="sub_kriteria" class="form-label">Sub Kriteria</label>
                        <input type="text" value="{{ $item->sub_kriteria }}" class="form-control" name="sub_kriteria">
                    </div>
                    <div class="mb-3">
                        <label for="bobot_sub" class="form-label">Bobot</label>
                        <input type="number" value="{{ $item->bobot_sub }}" class="form-control" name="bobot_sub">
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
